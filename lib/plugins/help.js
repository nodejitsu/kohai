...
help.start = function (client, config) {
  client.on("help", function (nick, command) {
    ...
    client.say(nick, "For more information, please see https://github.com/nodejitsu/kohai");
  });
}
...
help.show = function (client, nick, trigger) {
  switch(trigger) {
    ...
    case "stfu":
      ...
      break;
    case "report":
      client.say(nick, trigger + ": Reports a Twitter username as a spammer and blocks them.  Please use care.");
      break;
    case "config":
      client.say(nick, trigger + ": Syntax: config <add|rm|set|get|save> <path:to:config:property> <value>");
      break;
    case "tweetlimit":
      client.say(nick, trigger + ": Retrieves info from Twitter Stream API about current rate limit status."); 
      break;
    default:
