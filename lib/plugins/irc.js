...
irc.start = function (client, config) {
  ...
  if(auth) {
    ...
    if(auth) {
      ...
      client.on("rateChange", function (rate) { 
        //console.log(rate); 
      });
...
function emitRate(client, config) {
  ...
  Object.getOwnPropertyNames(config.plugins.irc.channels).forEach( function(channel, index) {
    client.on('message'+channel, function msgcounter(){
...
irc.rateMonitor = function (client, config, channel) {
  ...
  config.plugins.irc.channels[channel].interval = setInterval(function(){
    ...
    config.plugins.irc.channels[channel].rollingValues.push(config.plugins.irc.channels[channel].message_count);
    config.plugins.irc.channels[channel].message_count = 0;
    ...
    config.plugins.irc.channels[channel].chatRate = sum;
    //console.log('chat_rate ', chat_rate);
    client.emit('rateChange'+channel, config.plugins.irc.channels[channel].chatRate);
