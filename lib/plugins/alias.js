...
alias.start = function (client, config) {
  ...
  client.triggers = {
    ...
    'tweetlimit': function (channel, name, message) {
      twit.rateLimitStatus(function (data) {
        if (data.statusCode == 200) {
          client.say(channel, data.remaining_hits + ' of ' + data.hourly_limit + ' tweets left for this hour.');
        }
        else {
          console.log('Error connecting to Twitter API: '.red);
          console.log(data);
        }
      });
    },
    ...
    'report' : function (channel, name, message, targ) {
      async.series([
        function (callback) {
          twit.reportSpam(targ, function (result) {
            console.log(targ, ' has been reported as a spammer.');
            client.say(channel, 'I have reported '+targ+' to the Twitter API as a spammer.');
            callback(null, result);
          });
        },
        function (callback) {
          twit.createBlock(targ, function (result) {
            console.log(targ, ' has been blocked.');
            client.say(channel, targ + ' has been successfully blocked.');
            callback(null, result);
          });
        }
      ], callback);
  ...
  client.on('pm', function (nick, msg) {
    for (var i=0; i<whitelist.length; i++) {
      if (whitelist[i] == nick) {
        var command = msg.match(/^\+config\s(add|rm|set|get|save)\s?([\w\d#:\._]*)\s?(.*)/); 
        if (command) {
    ...
    client.on('message' + channel,function triggerListener(from, message) {
      for (var i=0; i<whitelist.length; i++) {
        if (whitelist[i] == from) {
          ...
          if ((trigger_match)&&(trigger_match[1] !== 'config')) {
