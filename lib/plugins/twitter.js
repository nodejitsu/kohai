...
twitter.start = function (client, config){
  ...
  Object.getOwnPropertyNames(config.plugins.irc.channels).forEach( function(channel, index) {
    client.on("rateChange"+channel, function (channel, rate) {
      ...
      if (config.plugins.irc.channels[channel].volume >= 10) config.plugins.irc.channels[channel].volume = 10;
      else if ((config.plugins.irc.channels[channel].volume < 0)||(typeof(config.plugins.irc.channels[channel].volume) === 'undefined')) config.plugins.irc.channels[channel].volume = 0;
          ...
          stream.on('data', function (data) {
            if(!beQuiet) {
              if((data.text)&&((!data.text.match(/.*\bRT:?.*/i))&&(!data.retweeted))) {
                Object.getOwnPropertyNames(config.plugins.irc.channels).forEach(function (channel, index) {
                  if (config.plugins.irc.channels[channel].volume > config.plugins.irc.channels[channel].currentTweetCount) {
