...
var triggers = module.exports = {
  ...
  'stfu' : function (data, command) {
    ...
    var self = this,
        dest = (isNaN(parseInt(command[2], 10))) ? command[2] : data.to,
        timer = (command[3] && !isNaN(parseInt(command[3], 10))) ? // If command[3] is a number,
          parseInt(command[3], 10) : // parse it as an int and use it.
            !isNaN(parseInt(command[2], 10)) ? // Otherwise, if command[2] is a number,
            parseInt(command[2], 10) : // try to parse command[2] as an int;
...
fs.readdirSync(__dirname + '/plugins').forEach(function (plugin) {
  var mod;
  if (/^[\w\s\d]+\.js$/.test(plugin)) {
    plugin = plugin.replace('.js', '');
    try {
      mod = require(__dirname + '/plugins/' + plugin);
      switch (typeof mod) {
      case 'function':
