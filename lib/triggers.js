...
var triggers = module.exports = {
  ...
  'tweet' : function (data, command) {
    ...
    this.emit('tweet', command.slice(1).join(' '));
  },

  'stopstream' : function (data, command) {
    if (!data.admin) {return false; }
    this.emit('stopTweets', null);
  },

  'startstream' : function (data, command) {
    if (!data.admin) { return false; }
  ...
  'insult' : function (data, command) {
    ...
    this.emit('sendMsg', { dest: data.to, msg: this.insults[n].replace('%%', command[1]) }) 
  },
